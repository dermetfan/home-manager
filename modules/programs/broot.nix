...
in {
  ...
  config = mkIf cfg.enable {
    ...
    programs.bash.initExtra = mkIf cfg.enableBashIntegration (
      ...
      mkAfter (builtins.readFile
        (pkgs.runCommand "br.bash" { nativeBuildInputs = [ cfg.package ]; }
          "broot --print-shell-function bash > $out")));

    programs.zsh.initExtra = mkIf cfg.enableZshIntegration (builtins.readFile
      (pkgs.runCommand "br.zsh" { nativeBuildInputs = [ cfg.package ]; }
        "broot --print-shell-function zsh > $out"));

    programs.fish.shellInit = mkIf cfg.enableFishIntegration (builtins.readFile
      (pkgs.runCommand "br.fish" { nativeBuildInputs = [ cfg.package ]; }
